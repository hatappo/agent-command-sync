...
> **Note:** `acs` および `agent-command-sync` コマンドは非推奨エイリアスとして引き続き利用可能です。
...
- **GitHub からダウンロード** — `asp download` で GitHub リポジトリからスキルを直接取得
- **上流からの更新** — `asp update` でダウンロード済みスキルの上流変更をチェックし適用
- **スキル一覧** — `asp list` で全エージェントのスキルを一覧表示
- **スキル情報表示** — `asp info` でスキルのメタ情報とソースリンクを確認
...
### `asp download <url> [to]`（エイリアス: `asp dl`）— GitHub からスキルをダウンロード

```bash
asp download https://github.com/anthropics/skills/tree/main/skills/skill-creator
asp download <url> gemini                  # Gemini のスキルディレクトリに配置
asp download <url> claude -g               # グローバル Claude ディレクトリに配置
asp download <url> -n                      # ダウンロードせずにプレビュー
```
...
### `asp update [skill-path]` — ダウンロード済みスキルを上流から更新

```bash
asp update                                 # 全エージェントスキルをチェック＆更新
asp update .claude/skills/my-skill         # 特定のスキルを更新
asp update skills/                         # パス配下の全スキルを更新
asp update -n                              # 更新チェックのみ（適用なし）
```

### `asp list`（エイリアス: `asp ls`）— スキル一覧を表示

```bash
asp list                                   # プロジェクトレベルのスキル一覧
asp list -g                                # グローバル（ユーザーレベル）のスキル一覧
```

### `asp info [skill-path]` — スキル情報を表示

```bash
asp info                                   # インタラクティブにスキルを選択して表示
asp info .claude/skills/my-skill           # スキル情報とソースリンクを表示
asp info .claude/skills/my-skill/SKILL.md  # SKILL.md パスの直接指定も可
```

### `asp sync <from> <to>` — エージェント間の直接変換

```bash
asp sync claude gemini                     # Claude → Gemini に変換
asp sync claude gemini -t commands          # Commands のみ
```

### `asp import <agent>` / `asp apply <agent>` — ロスレス変換のワークフロー

```bash
asp import claude                          # Claude → Chimera Hub にインポート
asp import gemini -t commands              # Commands のみインポート
asp apply gemini                           # Chimera Hub → Gemini に適用
asp apply claude --remove-unsupported      # サポートされていないフィールドを削除
```

### `asp drift <agent>` / `asp plan <agent>` — プレビュー（ドライラン）

```bash
asp drift claude                           # インポートの変更をプレビュー
asp plan gemini                            # 適用の変更をプレビュー
```

### `asp migrate` — Chimera Hub ディレクトリを .acs から .asp に移行

```bash
asp migrate                                # .acs → .asp にリネーム（ユーザーレベル + プロジェクトレベル）
```
...
| `--chimera-dir <path>`      | Chimera Hub ベースディレクトリ（デフォルト: ~/.config/asp）                 |
...
# Chimera ハブワークフロー
asp import claude                          # Claude → Chimera にインポート
asp import gemini                          # Gemini → Chimera にインポート（マージ）
asp apply claude                           # Chimera → Claude に適用（Claude extras 付き）
asp apply gemini                           # Chimera → Gemini に適用（Gemini extras 付き）
...
Git リポジトリ内で実行すると、`asp` はデフォルトで**プロジェクトレベル**のディレクトリ（例: `<repo>/.claude`、`<repo>/.gemini`）を使用します。ユーザーレベルのディレクトリを使用するには `-g`/`--global` を指定してください。
...
# プレビューして適用
asp plan codex                             # プレビュー
asp apply codex                            # 適用
```

ハブファイルは `~/.config/asp/`（グローバル）または `<repo>/.asp/`（プロジェクト）に保存されます。

詳細なワークフローと例については [Chimera Hub ワークフロー](docs/chimera-hub-workflow.md) を参照してください。

## Commands

`asp` はエージェント間での単一ファイルスラッシュコマンドの変換もサポートしています。コマンド形式の詳細、メタデータ比較、プレースホルダー構文については [Commands リファレンス](docs/commands-reference_ja.md) を参照してください。
