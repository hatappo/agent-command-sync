...
### Changed
- **Repository moved**: Moved from `hatappo/agent-command-sync` to `skill-mill/agent-command-sync` GitHub organization. All repository URLs updated.
- **README cleanup**: Removed version upgrade/migration notes from READMEs. Updated project description to be more concise.

## [5.6.1] - 2026-03-01

### Added
- **CLI demo animation**: Added animated SVG demos to README showcasing `download`, `sync`, `update`, `info`, and `status` commands. Built with `terminal-demo` + `svg-term-cli`. English and Japanese versions included (`demo/`).

### Changed
- **README images replaced**: Static screenshots replaced with animated SVG demo at the top of README.

## [5.6.0] - 2026-02-28

### Changed
- **Short SHA for `_from` provenance**: `_from` tree hash now defaults to 7-character short SHA (e.g., `owner/repo@d5cd683`) instead of full 40-character hash. Use `--full-hash` option on `download` and `update` commands to keep full hashes. Existing full hashes are still recognized via prefix matching.

## [5.5.0] - 2026-02-28

### Added
- **Interactive skill selection for `acs info`**: Running `acs info` without arguments discovers all skills across agent directories and presents an interactive arrow-key selection UI
  - Shows skill count, numbered choices with paths, and descriptions
  - Uses `@inquirer/select` for the selection UI
- **Two-level help display (Cargo pattern)**: `acs -h` shows concise help, `acs --help` shows detailed help with advanced commands and examples

### Changed
- **`acs info` argument is now optional**: `info <skill-path>` → `info [skill-path]`

## [5.4.0] - 2026-02-28

### Added
- **`acs info <skill-path>` subcommand**: Display skill information and source links
  - Shows metadata: description, source (`_from`), license, local path, Git remote URL
  - Lists files in the skill directory (direct files + subdirectories with file counts)
  - Displays source links (GitHub, skillsmp.com, skills.sh) when `_from` provenance exists
  - Uses GitHub API to resolve the actual skill path in the source repository
  - Accepts both skill directory path and `SKILL.md` file path
- **`getOriginRemoteUrl()` utility**: Returns the raw origin remote URL from `.git/config` (no GitHub normalization)
