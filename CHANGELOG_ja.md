...
### 変更
- **ドキュメント**: 全ドキュメント・ヘルプテキスト・デモアニメーションで `download` を `add` に統一
- **デモ**: 新しい `sk` CLI と `add` サブコマンドでデモ SVG/GIF を再作成
...
- パッケージ名を `agent-command-sync` から `agent-skill-porter` に変更
- CLI コマンドを `acs` から `asp` に変更（`acs` は非推奨エイリアスとして継続利用可能）
- Chimera Hub ディレクトリを `.acs`/`.config/acs` から `.asp`/`.config/asp` に変更（レガシー `.acs` ディレクトリは自動検出でフォールバック）

### 追加機能

- **`asp migrate` サブコマンド**: Chimera Hub ディレクトリを `.acs` から `.asp` にリネーム（ユーザーレベル + プロジェクトレベル、冪等）

## [5.8.0] - 2026-03-12

### 追加機能
- **`acs list` サブコマンド**（エイリアス: `acs ls`）: 全エージェントのスキルをエージェント別にグループ化して一覧表示。スキル名とパスを表示。`-g` でグローバル（ユーザーレベル）ディレクトリに対応。

### 修正
- **`acs info` インタラクティブ選択**: Escape キーでプロンプトを終了可能に（従来は Ctrl+C が必要だった）。

## [5.7.0] - 2026-03-06

### 変更
- **リポジトリ移動**: GitHub リポジトリを `hatappo/agent-command-sync` から `skill-mill/agent-command-sync` に移動。全リポジトリ URL を更新。
- **README 整理**: バージョンアップグレード・移行ガイドの記載を README から削除。プロジェクト説明文を簡潔に更新。

## [5.6.1] - 2026-03-01

### 追加機能
- **CLI デモアニメーション**: `download`, `sync`, `update`, `info`, `status` コマンドを紹介するアニメーション SVG を README に追加。`terminal-demo` + `svg-term-cli` で生成。英語・日本語版を `demo/` に収録。

### 変更
- **README の画像を差し替え**: 静的スクリーンショットをアニメーション SVG デモに置き換え。
