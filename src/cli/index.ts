...
program
  .requiredOption(
    '-c, --convert <direction>',
    'Conversion direction: c2g (Claude to Gemini) or g2c (Gemini to Claude)'
  )
  .option(
    '--remove-unsupported',
    'Remove keys that are not supported in the target format',
    false
  )
  .option(
    '--no-overwrite',
    'Skip conversion if a command with the same name exists in the target'
  )
  .option(
    '--sync-delete',
    'Delete commands in the target that don\'t exist in the source',
    false
  )
  .option(
    '-f, --file <filename>',
    'Convert only the specified command file (without extension)'
  )
  .option(
    '-d, --dry-run',
    'Display a list of changes without applying them',
    false
  )
  .option(
    '-v, --verbose',
  ...
  .option(
    '--gemini-dir <path>',
    'Gemini commands directory (default: ~/.gemini/commands)'
  )
