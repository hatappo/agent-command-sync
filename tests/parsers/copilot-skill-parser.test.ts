...
import type { CopilotSkill } from "../../src/types/index.js";
import { fileExists } from "../../src/utils/file-utils.js";

const fixturesPath = join(process.cwd(), "tests/fixtures/copilot-skills");

describe("CopilotAgent (Skill)", () => {
  ...
  describe("parseSkill", () => {
    it("should parse a skill directory", async () => {
      ...
      expect(result.description).toBe("A test Copilot skill");
      expect(result.content).toContain("test Copilot skill");
    ...
    it("should throw error for invalid skill directory", async () => {
      ...
      await expect(agent.parseSkill(invalidDir)).rejects.toThrow("Failed to parse Copilot skill");
    });
  });
  ...
  describe("stringifySkill", () => {
    it("should stringify a skill to SKILL.md format", async () => {
      ...
      expect(result).toContain("description: A test Copilot skill");
    });
  });
  ...
  describe("writeSkillToDirectory", () => {
    ...
    beforeEach(async () => {
      tmpDir = join(tmpdir(), `copilot-skill-write-test-${Date.now()}`);
    });
